      ...
      <Section tight>
        <div className="shell grid gap-10 lg:grid-cols-[1fr_1.4fr] lg:gap-16">
          ...
          <form aria-label="Add product (preview — not active)" data-reveal>
            <fieldset disabled className="border border-line bg-mist p-5 sm:p-8">
              <legend className="sr-only">Add product</legend>

              <div className="grid gap-5 sm:grid-cols-2">
                {FIELDS.map((f) => (
                  <label key={f.label} className="block">
                    <span className="text-label block text-muted">{f.label}</span>
                    <input
                      type={f.type}
                      placeholder={f.placeholder}
                      className="mt-2 w-full border border-line bg-canvas px-4 py-3 text-sm text-ink-soft placeholder:text-muted disabled:cursor-not-allowed"
                    />
                  </label>
                ))}
              </div>
              ...
              <label className="mt-5 block">
                <span className="text-label block text-muted">Description</span>
                <textarea
                  rows={3}
                  placeholder="Two or three lines a buyer would actually read."
                  className="mt-2 w-full resize-none border border-line bg-canvas px-4 py-3 text-sm text-ink-soft placeholder:text-muted disabled:cursor-not-allowed"
                />
              </label>

              <div className="mt-5 flex flex-col items-start gap-3 border border-dashed border-line-strong bg-canvas px-5 py-8 text-center sm:items-center">
                <span className="font-sans text-[0.6875rem] font-semibold uppercase tracking-[0.14em] text-muted">
                  Drop photographs here
                </span>
                <span className="text-xs text-muted">JPG or PNG · resized and compressed automatically</span>
              </div>
            ...
            <p className="mt-4 font-sans text-[0.625rem] font-semibold uppercase tracking-[0.12em] text-muted">
        ...
        <div className="shell">
          ...
          <ul className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4" data-reveal>
            {lookEntries.map(([slug, meta]) => {
              ...
              return (
                <li key={slug} className="flex flex-col border border-line bg-mist">
                  {img && (
                    <Image
                      src={img}
                      alt=""
                  ...
                  <div className="flex flex-1 flex-col p-4">
                    <span className="text-[0.9375rem] font-medium text-ink">{meta.name}</span>
                    <span className="mt-1 text-xs text-muted">{meta.category}</span>
                    <span className="mt-1 text-xs text-body">{meta.detail}</span>
                    <div className="mt-4 flex gap-2 pt-1">
                      <span className="flex-1 cursor-not-allowed border border-line px-3 py-2 text-center font-sans text-[0.625rem] font-semibold uppercase tracking-[0.12em] text-muted">
                        Edit
                      </span>
                      <span className="flex-1 cursor-not-allowed border border-line px-3 py-2 text-center font-sans text-[0.625rem] font-semibold uppercase tracking-[0.12em] text-muted">
                        Remove
          ...
          <p className="mt-6 font-sans text-[0.625rem] font-semibold uppercase tracking-[0.12em] text-muted">
