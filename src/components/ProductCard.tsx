...
export function ProductCard({
  ...
  return (
    <li className="group relative flex flex-col border border-line bg-mist transition-colors duration-500 focus-within:border-line-strong hover:border-line-strong">
      <div className="relative overflow-hidden bg-sand">
        ...
        <span className="text-label absolute left-0 top-0 bg-canvas/85 px-2.5 py-1.5 text-accent backdrop-blur-sm sm:px-3 sm:py-2">
          {String(index + 1).padStart(2, '0')}
        </span>
      </div>

      <div className="flex flex-1 flex-col p-3.5 sm:p-5 lg:p-6">
        <h3 className="text-d3 !text-[0.9375rem] leading-tight text-ink sm:!text-[1.15rem] lg:!text-[1.25rem]">
          <Link href={`/products/${product.slug}`} className="transition-colors group-hover:text-accent">
        ...
        <p className="mt-2 line-clamp-2 text-[0.8125rem] leading-relaxed text-body sm:mt-3 sm:line-clamp-3 sm:text-sm">
          {product.intro}
        </p>
        ...
        >
          {tags.map((t, i) => (
            <li
              ...
              className={`border border-line px-2 py-1 font-sans text-[0.5625rem] font-semibold uppercase tracking-[0.12em] text-muted sm:px-2.5 sm:text-[0.625rem] sm:tracking-[0.1em] ${
                i === 2 ? 'hidden sm:block' : ''
              }`}
            >
              {t}
        ...
        <span className="link-arrow mt-auto pt-4 !text-[0.5625rem] text-ink-soft transition-colors group-hover:text-accent sm:pt-6 sm:!text-[0.6875rem]">
